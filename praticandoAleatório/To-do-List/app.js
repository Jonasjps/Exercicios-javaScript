...
const alterandoClass = (todos, classAdd, classRemove) => {
    todos.forEach(todo => {
...
const hiDetodos = (todos, filtrando) => {
   const falseTodos = filterTodos(todos, filtrando, false)
    alterandoClass(falseTodos, 'hedden', 'd-flex')
    
}

const showTodos = (todos, filtrando) => {
    const trueTodos = filterTodos(todos, filtrando, true)
    alterandoClass(trueTodos, 'd-flex', 'hedden')
