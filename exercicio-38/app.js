...
const messageError = errorType => ({
  'unsupported-code': 'Não suportamos o código de moedas fornecidas.',
  'malformed-request': 'Alguma parte da sua solicitação não segue a estrutura padrão: "https://v6.exchangerate-api.com/v6/YOUR-API-KEY/latest/USD "',
  'invalid-key': 'Sua chave API não é valida.',
  'inactive- account': 'O seu indereço de email não foi confirmado.',
  'quota-reached': 'Sua conta atingiu o número de solitações permitida pelo seu plano.'
})[errorType]

const exchangeRates = async () => {
try {
    const getConversionRates = await fetch(exchangeUrl('BRL' ))
