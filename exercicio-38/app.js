...
const correncyContainerEl = document.querySelector('[data-js="currency-container"]')

const getCurrencyUrl = currency => 
  `https://v6.exchangerate-api.com/v6/04cf6b5908dbe464ff892035/latest/${currency}`

const getMessageErro = typeError => ({
...
const getConversionRates = async () => {
  try{
    const fetchCurrency = await fetch(getCurrencyUrl('LLL'))

    if(!fetchCurrency.ok) {
      throw new Error('Sua conexão falhou. Não foi possível obter as informações.')
    }

    const conversionRates = await fetchCurrency.json()

    if(conversionRates.result === 'error') {
      throw new Error(getMessageErro(conversionRates['error-type']))
    }
    ...
    correncyContainerEl.insertAdjacentElement('afterend', div)
