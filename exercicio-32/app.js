...
const insertGIFIntoDOM = async inputValue => {
  const GIFs = await fetchGIF(inputValue)
  
  if(GIFs) {
    const GIFSUrl = GIFs.data[0].images.downsized.url
    const img = setImageGIF(GIFSUrl, GIFs)
