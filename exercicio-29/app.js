...
const logPokemon = (error, data) => error 
  ? console.log(error) 
  : console.log(`Pokémon obtido: ${data.name}`)

const idPokemonData = id => `https://pokeapi.co/api/v2/pokemon/${id}`
const bulbasaur = idPokemonData(1)
const charmander = idPokemonData(4)
const squirtle = idPokemonData(7)

getPokemon(bulbasaur, (error, data) => {
  logPokemon(error, data)
  getPokemon(charmander, (error, data) => {
    logPokemon(error, data)
    getPokemon(squirtle, logPokemon)
  })
})
/*
  02
...
const map = (array, callback) => {
  ...
  const createNewMap = number => {
    const newMapArray = callback(number)
    newArray.push(newMapArray)
  }

  array.forEach(createNewMap)
  return newArray
}

console.log(map([1, 2, 3], number => number * 2))
console.log(map([2, 3, 4], number => number * 3))
/*
  03
...
const frequenceAgeCurtoCircuito = (acc, {age}) => {
  acc[age] = acc[age] + 1 || 1
  return acc
}

const frequenceAges = people.reduce(frequenceAgeCurtoCircuito, {})
